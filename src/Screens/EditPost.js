...
const EditPost = ({ navigation, route }) => {
    ...
    const savePost = () => {
        ...
        var url = `http://localhost:3333/api/1.0.0/user/${route.params.user_id}/post/${route.params.post_id}`;
        var headers = {
        ...
        <ScrollView contentContainerStyle={{ flex: 1, backgroundColor: 'red' }}>
            <View style={styles.container}>
                ...
                <TouchableOpacity disabled={postText.length == 0 ? true : false} onPress={() => savePost()}
                    ...
                    <Text style={{ color: postText.length == 0 ? '#828282' : '#fff', fontSize: 20, fontWeight: 'bold' }}> Save </Text>
                </TouchableOpacity>
